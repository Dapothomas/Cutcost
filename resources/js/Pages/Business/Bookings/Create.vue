        ...
        <div class="page-shell max-w-xl">
            <form class="card space-y-4 p-6" @submit.prevent="submit">
                <div class="space-y-2">
                    ...
                    <select id="client_id" v-model="form.client_id" name="client_id" required class="form-input">
                        <option value="" disabled>Select client</option>
                        <option v-for="client in clients" :key="client.id" :value="client.id">{{ client.name }}</option>
                    </select>
                    <p v-if="form.errors.client_id" class="text-sm text-destructive">{{ form.errors.client_id }}</p>
                    <Link href="/business/clients/create" class="text-sm text-muted-foreground hover:text-foreground">+ Add new client</Link>
                </div>

                <div class="space-y-2">
                    <label for="service_id" class="text-sm font-medium">Service <span class="text-destructive">*</span></label>
                    <select id="service_id" v-model="form.service_id" name="service_id" required class="form-input">
                ...
                <div class="space-y-2">
                    ...
                    <select id="barber_id" v-model="form.barber_id" name="barber_id" required class="form-input">
